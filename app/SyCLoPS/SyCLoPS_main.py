...
TEMPESTEXTREMESDIR = os.path.expanduser('/g/data/tp28/dev/eh6215/low_tracking/tempestextremes_missingvalue/bin')
...
RangeDist = 6.000001 #StitchNodes argument. 
...
# Mode choice by user
while True:
    mode="Y"#2#input("Do you want to classify tropical-like cyclones (y/n)? Default is 'y'. If you only focus on tropical systems or typical extratropical systems, you may enter 'n' to save time: ")
    if mode == '':
        mode='Y'
    if mode in ['Y', 'N', 'y', 'n']:
...
inputfile_DN = "input.txt" #"This txt file should contain the list of existing input files that contains all the necessary variables for DetectNodes. \
# Please refer to "Manual/Required_Variables.png" for the list of required variables.
outputfile_DN = "output_DN.txt" #This txt file should contain a list of output path and filenames that will be generated by DetectNodes and should have the same number of lines as the inputfiles.
outputfile_SN = "lps_tracks.csv" #This is the single output track file's filename in StitchNodes.
inputfile_VP = "input_VP.txt" #This txt file should contain the list of U and V at 850 hPa and 925 hPa levels.
outputfile_VP = "output_VP.txt" #This txt file should contain the list of output cyclonic relative vorticity path and filenames that will be generated by VariableProcessor \
# and should have the same number of lines as the inputfiles.
outputfile_DB = "sizeblob_out.txt" #This txt file should contain the list of output path and filenames that will be generated by DetectBlobs and should have the same number of lines as outputfile_VP.
outputfile_BS = "size_blob_stats.txt" #This is the filename of the single output file of BlobStats which contains the blob statistics.
...
detect_nodes_cmd = [
    ...
    "--searchbymin", "psl",
    "--closedcontourcmd", "psl,10,5.5,0",
    "--mergedist", str(MergeDist),
    "--outputcmd", #Output the following parameters for classification.
    "psl,min,0;" #MSLP
    "psl,posclosedcontour,2.0,0;" #MSLPCC20
    "psl,posclosedcontour,5.5,0;" #MSLPCC55
    #"wsgsmax,max,2.0;" #WG
    #"sfcWind,max,2.0;" #WS
    #"pr,avg,5.0;"  #PRECIP
    #"_VECMAG(VAR_10U,VAR_10V),max,2.0;" #WS ##This is an optional parameter for reference purpose.
    "_DIFF(_VECMAG(ua200,va200),_VECMAG(ua850,va850)),avg,10.0;" #DEEPSHEAR 
    ##If variables in your inputfiles are on a single level, e.g., U200 representing U at 200 hPa, then directly use "U200."
    "_DIFF(zg300,zg500),negclosedcontour,6.5,1.0;" #UPPTKCC
    "_DIFF(zg500,zg700),negclosedcontour,3.5,1.0;" #MIDTKCC
    "_DIFF(zg700,zg925),negclosedcontour,3.5,1.0;" #LOWTKCC
    "zg500,posclosedcontour,3.5,1.0;" #Z500CC
    "_CURL{16,2.5}(ua500,va500),max,0;" \
    ...
    "ta100,max,0.0;" #T850
    "ta850,max,0.0;" #T850
    "zg850,min,0;" #Z850 ##This parameter is not necessary if the dataset contains NaNs or missing values where 850 hPa level is below the surface.
    "ua850,posminusnegwtarea,5.5;" #U850DIFF
    "_VECMAG(ua200,va200),maxpoleward,1.0", #WS200PMX
...
stitch_nodes_cmd = [
    ...
    "--in_fmt", "lon,lat,MSLP,MSLPCC20,MSLPCC55,WS,DEEPSHEAR,UPPTKCC,MIDTKCC,LOWTKCC,"
                "Z500CC,VO500AVG,Q100,Q850,T100,T850,Z850,U850DIFF,WS200PMX",
...
variable_processor_cmd = [
    ...
    "--var", "_COND(_LAT(),_CURL{8,3}(ua850,va850),_PROD(_CURL{8,3}(ua850,va850),-1)),ua925,va925",
        ...
        if step <= 3:
            ...
            if use_srun:
                ...
                if "EXCEPTION" in last_line:
                    ...
                    check_log_dir(log_dir,t0)
                """
                print("Skipping Variable Processor - EH")
            t1 = time.time()
        ...
        if step <= 4:
            ...
            if use_srun:
                ...
                if "EXCEPTION" in last_line:
                    ...
                    check_log_dir(log_dir,t0)
                """
                print("Skipping Detect Blobs - EH")
            t1 = time.time()
...
confirm='y'#input("Please review the code and comments in SyCLoPS_classifier.py and modify accordingly before running. Are you ready to run SyCLoPS_classifier.py now? (y/n). Default is 'n': ")
if confirm.lower() != 'y':
