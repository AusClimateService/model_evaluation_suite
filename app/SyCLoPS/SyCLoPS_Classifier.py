...
print('Please change the constants and files names accordingly in the first part of the script before running the program.\n')
import pandas as pd 
import numpy as np
import xarray as xr
import math
...
TETrackFile='lps_tracks.csv' #The StitchNodes output file in CSV format. Required before running the program.
...
SizeBlobStatFile='size_blob_stats.txt' #Enter a filename for the output of TE's BlobStats. 
...
ClassifiedOutFile_CSV='SyCLoPS_classified.csv' #Choose the output filename of the final classified dataset.
ZSFile='orog.nc' #The ERA5 invariant surface geopotential file (0.25 deg) which can be used for any datasets in the classifier. No need to change this.
#------------------Functions--------------------#
...
def track_percor(k): #Calculates the Pearson correlation coefficient of all the nodes within a track
    with warnings.catch_warnings():
        ...
        percor=abs(stats.pearsonr(LON[FST[k]:LST[k]+1], LAT[FST[k]:LST[k]+1])[0])
...
# Convert specific humiditiy to water vapour pressure
def convert_q2e(q, p, Mw=18.01528, Md=28.9644):
    """
    Convert specific humidity to water vapour pressure
    :param q: specific humidity in kg/kg (xarray)
    :return: xarray containing water vapour pressure in Pa
    """
    r = Mw / Md
    with xr.set_options(keep_attrs=True):
        return p*q / (r + (1-r)*q)

# Define the saturation function (Ambaum) with regards to water/ice
def esat_ambaum(T, oname = 'water_vapour_pressure'):
    """
    Calculates saturation water vapour pressure (wrt water or ice) using Ambaum, Maarten HP. "Accurate, simple equation for saturated vapour pressure over water and ice." Quarterly Journal of the Royal Meteorological Society 146.733 (2020): 4252-4258.
    :param T: temperature in K (xarray)
    :return: xarray containing saturation water vapour pressure in Pa
    """
    # Define constants
    T0=273.16 # Reference temperature
    e0=611.655 # Water vapour pressure at T0
    cplmcpv=2180 # isobaric heat capacity difference of liquid to vapour
    cpimcpv=212 # isobaric heat capacity difference of ice to vapour
    L0=2.501e6 # Latent heat of evaporation
    Lm0=0.33344e6 # Latent heat of melting
    Ls0=L0+Lm0 # Latent heat of sublimation
    Rv=461.52 # specific gas constant for water vapour
    # Do calculations
    esat = T.apply(lambda t:__est_ambaum(t,e0,T0,cplmcpv, Rv,[Ls0,L0][int(t>T0)]))
    return esat



def __est_ambaum(T, e0, T0, cpXmcpv, Rv, L0):
    """
    Calculates saturation water vapour pressure (wrt water or ice) using Ambaum, Maarten HP. "Accurate, simple equation for saturated vapour pressure over water and ice." Quarterly Journal of the Royal Meteorological Society 146.733 (2020): 4252-4258.
    :param T: temperature in K (xarray)
    :param e0: Water vapour pressure at T0
    :param T0: Reference temperature
    :param cpXmcpv: isobaric heat capacity difference of liquid/ice to vapour
    :param Rv: specific gas constant for water vapour
    :param L0: Latent heat of evaporation
    :return: xarray containing saturation water vapour pressure in Pa
    """
    L = latent_heat(T, T0=T0, cpXmcpv=cpXmcpv, L0=L0)
    return e0 * np.power(T0/T, cpXmcpv/Rv) * np.exp(L0/(Rv*T0)-L/(Rv*T))

def latent_heat(T, T0, cpXmcpv, L0):
    """
    Calculates latent heat of evaporation (sublimation) according to Ambaum, Maarten HP. "Accurate, simple equation for saturated vapour pressure over water and ice." Quarterly Journal of the Royal Meteorological Society 146.733 (2020): 4252-4258.
    :param T: temperature in K (xarray)
    :param T0: Reference temperature
    :param cpXmcpv: isobaric heat capacity difference of liquid/ice to vapour
    :param L0: Latent heat of evaporation
    :return: xarray containing latent heat
    """
    return L0 - cpXmcpv * (T-T0)
...
if __name__ == '__main__':
    ...
    print(long_str)
    while True:
        modenum = 0#input("Enter a mode number (leaving blank is 0):")
        if modenum == '':
            modenum = 0
        else:
            modenum = int(modenum)
    ...
    print(long_str)
    while True:
        unitnum = 2#input("Enter a number (Default is 1):")
        if unitnum == '':
            unitnum = 1
        else:
            unitnum = int(unitnum)
    ...
    while True:
        data250="N"#input("Enter Y/y (Yes) or the DEFAULT N/n (No):")
        if data250 == '':
            data250='N'
    ...
    while True:
        isregion="Y"#input("Enter Y/y (Yes) or the DEFAULT N/n (No):")
        if isregion == '':
            isregion='N'
    ...
    while True:
        timeres = 6# input ("Enter a value (leaving blank is the default '3'):")
        if timeres == '':
            timeres=3
        else:
            timeres=int(timeres)
    ...
    if modenum<=3:
        ...
        dszs=xr.open_dataset(ZSFile).orog
        lonz=dszs.lon.to_numpy()%360*(np.pi/180)
        latz=dszs.lat.to_numpy()%360*(np.pi/180)
        ...
        if modenum==0 or modenum==1:
            ...
            nodepair_list=[blobpairing(x) for x in range(len(BlobTime))]
            zsperl=[zsper(x) for x in range(len(dfin))]
            #processing the output tuple list and pair LSP nodes to blobs.
            nodepair_list=sum(nodepair_list,[])
        ...
        if modenum==0 or modenum==2:
            ...
            distspr_list = [track_spread(i) for i in range(len(FST))]
            percor_list = [track_percor(i) for i in range(len(FST))]
            zmax_list = [zsmax(i) for i in range(len(dfin))]
    ...
    dfin['RH850AVG'] = convert_q2e(dfin.Q850, 85000, Mw=18.01528, Md=28.9644) \
                       / esat_ambaum(dfin.T850) * 100
    dfin['RH100MAX'] = convert_q2e(dfin.Q100, 10000, Mw=18.01528, Md=28.9644) \
                       / esat_ambaum(dfin.T100) * 100
