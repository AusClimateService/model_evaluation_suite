...
long_names = {
    ...
    'tasmean': 'Daily Mean Near-Surface Air Temperature'
}
...
def fix_input_metadata(ds, variable, time_agg):
    ...
    if 'DailyPrecip' in ds.variables:
      ds = ds.rename({'DailyPrecip':variable})
    
    if variable in ['pr', 'precip', 'mtpr', 'tp','DailyPrecip']:
        cf_var = 'pr'
    elif variable in ['tasmax', 'tmax', 'mx2t']:
        cf_var = 'tasmax'
    elif variable in ['tasmin', 'tmin', 'mn2t']:
        cf_var = 'tasmin'
    elif variable in ['tasmean']:
        cf_var = 'tasmean'
...
#        ds[cf_var].attrs['units'] = 'mm d-1'
    ...
    if units in ['kg m-2 s-1']:
       ds[variable] = ds[variable]*3600
       ds[variable].attrs['units'] = 'mm h-1'
    if units in ['K']:
...
def main(args):
    ...
    if 'day' in args.index_name:
       index = index.rename({args.index_name:args.index_name.replace('day','hour')})
       args.index_name = args.index_name.replace('day','hour')
    index.attrs['icclim_version'] = icclim.__version__
    index.to_netcdf(args.output_file,encoding={args.index_name:{'zlib':True},
