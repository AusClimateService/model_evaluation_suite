...
https://github.com/AusClimateService/model-evaluation/blob/master/spatial_selection.py (Damien Irving)

Basic Usage:
import geopandas as gp
nrm_clusters = gp.read_file('/g/data/tp28/dev/evaluation_datasets/NRM_clusters/NRM_clusters.shp')
lon = numpy 1D array of lon
lat = numpy 1D array of lat
...
def select_shapefile_regions(
    ...
    drop bool, default True
        Remove all fully masked rows and columns from output dataset
    Returns
    -------
    ds : xarray DataArray or Dataset
...
def fraction_weight_mask(shapes_gp, lons, lats):
    """Create a 3D float array for grid cells of fraction weights.
    ...
    mask_3D : xarray DataArray
        Three dimensional (i.e. region/lat/lon) float array
    """
...
def _check_regular_grid(dim_values):
    ...
    assert math.isclose(
        min_spacing, max_spacing, rel_tol=1e-3
